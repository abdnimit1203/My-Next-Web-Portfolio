...
# Hero Photo Design Versions

This document preserves the code and instructions for switching between **Version 1** and **Version 2** of the Hero Photo component (`src/components/sections/HeroPhoto.tsx`).

---

## Version 1: Custom Glowing Orb + `hero-DP.jpg` (Active Version)

Uses `/hero-DP.jpg` with an enlarged circular frame, layered over the custom glowing orb background (`/hero-circle-bg.png`) flipped horizontally (`-scale-x-100`) so the left side features cool cyan/blue glow tones.
...
export function HeroPhoto() {
  return (
    <div className="relative mx-auto flex aspect-square w-full max-w-[480px] items-center justify-center p-2 sm:p-4">
      ...
      {/* Main Avatar / Subject Container (Enlarged) */}
      <motion.div
        animate={{ y: [0, -6, 0] }}
...
## Version 2: Original Portrait Photo + `hero-photo.jpg`

Uses `/hero-photo.jpg` with the glowing background orb or gradient backdrop.
...
export function HeroPhoto() {
  return (
    <div className="relative mx-auto flex aspect-square w-full max-w-md items-center justify-center p-4">
      {/* Background Soft Glow */}
      <div className="absolute top-[12%] h-[74%] w-[74%] rounded-full bg-gradient-to-r from-cyan-500/40 via-purple-600/35 to-pink-500/35 blur-2xl animate-pulse pointer-events-none" />
      ...
      {/* Main Avatar / Subject Container (using hero-photo.jpg) */}
      <motion.div
        animate={{ y: [0, -6, 0] }}
        transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
        className="relative z-10 h-4/5 w-4/5 overflow-hidden rounded-full border-2 border-white/25 shadow-2xl shadow-slate-950/90"
      >
        <Image
          src="/hero-photo.jpg"
          alt="Abdullah Ibne Ali"
